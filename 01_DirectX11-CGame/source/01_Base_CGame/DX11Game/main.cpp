...
#define WINDOW_NAME		_T("個人制作")		// ウインドウのキャプション名
...
int							g_bDebugVisible;		// デバッグ表示のフラグ
...
void Draw(void)
{
	...
	else if (CInput::GetKeyRelease(VK_C))
	{
		g_bDebugVisible = 0;
	}

	if (g_bDebugVisible == 1)
	{
		// デバッグ文字列表示
		CDebugProc::Draw();
	}
