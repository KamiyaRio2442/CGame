			...
			case 3:	// 三人称カメラ
				{
					XMMATRIX mW = XMLoadFloat4x4(&m_pPlayer->GetWorld());
					XMStoreFloat3(&m_vUp, XMVector3TransformNormal(XMVectorSet(0,1,0,0), mW));
					XMStoreFloat3(&m_vTarget, XMVector3TransformCoord(XMVectorSet(0.0f, 80.0f, 0.0f, 1.0f), mW));
					XMStoreFloat3(&m_vPos, XMVector3TransformCoord(XMVectorSet(0.0f, 100.0f, -300.0f, 1.0f), mW));
				}
				break;
		}
	}
...
void CCamera::Clear()
{
	...
	pDC->ClearDepthStencilView(g_pDepthStencilView, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);
...
// プレイヤーをセット
void CCamera::SetPlayer(CCard* pPlayer)
{
	m_pPlayer = pPlayer;
...
int CCamera::CollisionViewFrustum(XMFLOAT3 vCenter, float fRadius)
{
	bool bHit = false;
...
void CCamera::UpdateViewFrustum()
{
	...
	// ※逆行列の転置行列を求めるのはXMPlaneTransformの仕様!!
	for (int i = 0; i < 6; ++i)
	{
