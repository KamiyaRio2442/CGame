	...
	const D3D11_INPUT_ELEMENT_DESC* pInpElemDesc, UINT uNumElem)
{
	...
	{
		_wsplitpath_s(pwszVSFName, wszDrive, _countof(wszDrive), wszDir, _countof(wszDir), wszFName, _countof(wszFName), nullptr, 0);
		...
		else 
		{
			...
			hr = D3DCompileFromFile(wszPath, nullptr, nullptr, "main", "vs_5_0", 0, 0, &pCompiledShader, nullptr);
			if (FAILED(hr)) 
			{
				WCHAR wszMsg[_MAX_PATH * 2];
...
{
	WCHAR wszPath[_MAX_PATH], wszDrive[_MAX_DRIVE], wszDir[_MAX_DIR], wszFName[_MAX_FNAME], wszCSODir[_MAX_DIR];
	HRESULT hr = S_OK;
	ID3DBlob* pCompiledShader = nullptr;
	PBYTE pbData = nullptr;
	long lSize = 0L;
	...
	{
		_wsplitpath_s(pwszPSFName, wszDrive, _countof(wszDrive), wszDir, _countof(wszDir), wszFName, _countof(wszFName), nullptr, 0);
		wcscpy_s(wszCSODir, _countof(wszCSODir), wszDir);
		wcscat_s(wszCSODir, _countof(wszCSODir), g_pszCSODir);
		_wmakepath_s(wszPath, _countof(wszPath), wszDrive, wszCSODir, wszFName, L".cso");
		FILE* fp = nullptr;
		...
		else 
		{
			...
			// ブロブからピクセルシェーダ作成
			_wmakepath_s(wszPath, _countof(wszPath), wszDrive, wszDir, wszFName, L".hlsl");
			hr = D3DCompileFromFile(wszPath, nullptr, nullptr, "main", "ps_5_0", 0, 0, &pCompiledShader, nullptr);
			if (FAILED(hr))
			{
				WCHAR wszMsg[_MAX_PATH * 2];
...
HRESULT LoadShader(LPCWSTR pwszVSFName, LPCWSTR pwszPSFName, ID3D11VertexShader** ppVertexShader, ID3D11InputLayout** ppVertexLayout, ID3D11PixelShader** ppPixelShader, const D3D11_INPUT_ELEMENT_DESC* pInpElemDesc, UINT uNumElem)
{
	// 頂点シェーダ読み込み
...
HRESULT LoadVertexShader(LPCSTR pszVSFName, ID3D11VertexShader** ppVertexShader, ID3D11InputLayout** ppVertexLayout, const D3D11_INPUT_ELEMENT_DESC* pInpElemDesc, UINT uNumElem)
{
...
HRESULT LoadShader(LPCSTR pszVSFName, LPCSTR pszPSFName, ID3D11VertexShader** ppVertexShader, ID3D11InputLayout** ppVertexLayout, ID3D11PixelShader** ppPixelShader, const D3D11_INPUT_ELEMENT_DESC* pInpElemDesc, UINT uNumElem)
{
